...
`/agent-session-*` 两个屏的输入条形态在 **0.9.2** 对齐了 WorkBuddy(单胶囊 +
语音 + 图片),之前那版是「输入框 + 框外圆按钮」。
...
24. **新建会话(0.9.0)**: 会话列表顶部「新建会话」大按钮 → 直接进会话详情,
    顶栏标题是「未命名会话」,消息区显示居中大图标 + 「有什么可以帮你的？」
25. **原生会话详情 - 历史(0.9.0)**: 点一条已有会话 → 原生渲染完整历史:
    用户消息靠右气泡、助手正文靠左(``` 代码块用等宽底框)、思考过程折叠卡
    (默认收起,点开展开)、工具调用折叠卡(名称 + 状态 chip,点开看入参/输出)
