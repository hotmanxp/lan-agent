...
官方机器人形象),并给助手正文接了真正的 **Markdown 渲染**;
**0.10.2** 把亮色主题的品牌色改回**平安橙 `#ff6600`**(0.10.0 临时用 WorkBuddy
青绿 `#0CC8A6`,现改回 zai `/m` 的 AI-Agent 头像家族色;深色主题不变)。
**0.10.3** 把运行态徽标(原本 `运行中`/`重试中` 文字 + 主题色圆环,稍重)改
成**左对齐 + 三点小点波浪动画 + 灰底**,只做轻提示,不抢输入框注意力。
**0.10.1** 再修两处「还是不像」的地方:**用户气泡改中性浅灰**(不是品牌绿)、
**输入条改双行白卡**(上排文本域 / 下排工具条,发送钮常驻)。

## 0.15.0 — 任务栏默认就是原生 Agent

任务栏不再是「入口卡片列表」,打开就直接进 **原生 Agent 工作区**:落在你**上次连接的那个 opencc 实例**的**最近一条会话**上;那个实例下线了,自动落到其他存活实例中的第一个。

- **会话切换面板**(左侧抽屉,点顶栏 ☰):
  - 顶部是**当前实例行**(图标 + 实例名 + 在线/离线 + `⌄`),点开「**选择实例**」底部弹层 —— 列出所有 opencc 实例,在线的带绿色 tag、离线的灰色 tag、当前那个打对勾;
  - 下面是**该实例的会话列表**(标题 / 模型 / 相对时间 / session id),点一条原地切换,不用退回上一页;
  - 「新建会话」pill 常驻;当前实例离线时压暗不可点。
- **实例从哪来**:优先 `/api/instances`(supervisor 快照,`state=running` 且有端口 = 在线);管理器不可达时回落成「入口卡片 URL 逐个探活」,保证管理器挂了活着的实例照样能聊。
- **入口卡片 + 扫码搬到设置栏**:任务栏不再承载它。它仍是「实例管理器地址」与兜底实例目录的数据源,增删改 / 拖拽 / 双按钮 / 扫码一个没少。
- 原来的「进行中」跨实例聚合**不再有 UI 入口**(任务栏按实例分家后它失去了位置),数据层保留。
- 「实例」栏「还没有可用实例管理器」的引导按钮指向**设置栏**(入口卡片搬到那儿了)。

## 0.14.1 — 图标统一为圆润风格

全 App 图标从 `Filled` / `Default` / `Outlined` 混用统一成 **`Icons.Rounded`**
(Material Symbols Rounded 那一套:转角和笔画端点全圆),底栏在 23dp 下差别最明显。

- 底栏**不再用「实心 / 描边」两套图标**:选中态改为同一个图标 + 深色 + SemiBold,
  未选中是 `onSurfaceVariant` 灰 + 78% 不透明度 —— 形状不变,切换时不会"跳"一下。
- 顶栏、卡片、实例页、终端页、Markdown 正文里的图标一并统一。
...
| 次要文字 | `#8C8C8C` | `#9AA0A8` |
| 发丝线 | `#EBEDF0` | `#2B2D31` |
| 品牌平安橙(主按钮 / 发送 / 运行中 / 启动图标底) | `#ff6600` | `#35D6B6` |
...
- 启动图标:品牌平安橙底 + 机器人头(`mipmap-anydpi-v26` 自适应图标,前景
...
品牌平安橙只留给发送按钮 —— 这是 WorkBuddy 与「绿色气泡 IM」的分水岭。
...
**入口 Card 列表**(0.15.0 起在**设置栏 → 入口卡片**;DataStore 持久化)有 3 种改法:

1. **APP 内编辑模式** — 设置栏「入口卡片」区右上 `✎` 进编辑模式,长按拖拽换位,点 `🗑` 删除,
   点 `+` 加新卡。改完即时写 DataStore,下次启动还是这些。
2. **QR 扫码进入** — 设置栏「入口卡片」区右上 QR 图标(CameraX + ML Kit Barcode),扫 zai 分享的 URL
